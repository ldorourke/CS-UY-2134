/*
 ...
 Creatin a binary search tree class
 */
...
public:
    ...
    //Problem 1b driver function
    void rangedPrint(const Comparable& low, const Comparable& high) {
        rangedPrint(root, low, high);
    }
    
    
    //Problem 1c driver functions
    void stringy() {
        vector<Node*> vec;
        stringy(vec, root); //this will add each node to the vector in order
        for (int i = 0; i < vec.size() - 1; i++) {
            vec[i]->left = nullptr; //set the left side of each node to nullptr
            vec[i]->right = vec[i+1]; //set the right side of each node to the next value in the vector
        }
        vec[vec.size()-1]->left = nullptr; //set last value's left side to nullptr
        swap(root, vec[0]);
    }
    ...
    double average_node_depth() const {
        ...
        return (double)average_node_depth(root, 0) / size;
    }
    
    
    
    /*
     Problem 2 - Level order traversal runs in worst case O(n)
     because it loops through everything on the queue until it is empty
     */
    void levelOrderTraversal() const {
        queue<Node*> printStuff;
        printStuff.push(root);
        while (printStuff.size() != 0) {
            cout << printStuff.front()->element << " ";
            if(printStuff.front()->left != nullptr) {
                printStuff.push(printStuff.front()->left);
            }
            if(printStuff.front()->right != nullptr) {
                printStuff.push(printStuff.front()->right);
            }
            printStuff.pop();
        }
    }
...
private:
    ...
    //Part 1a - Contains method recursively
    ...
    // Problem 1b - Print in range
    void rangedPrint(Node* t, const Comparable& low, const Comparable& high) const {
        if (t == nullptr) {
            return;
        }
        if (t->element >= low && t->element <= high) {
            rangedPrint(t->left, low, high);
            cout << t->element << " ";
            rangedPrint(t->right, low, high);
        }
    ...
    //Problem 1c - stringy creating unbalanced tree
    void stringy(vector<Node*>& v, Node* t) { //simple in order traversal adding each value to the vector
        if (t->left != nullptr) {
            stringy(v, t->left);
        }
        v.push_back(t);
    ...
    //Problem 1d - average node depth
    int average_node_depth( Node* node,  int level) const {
        if (node->left == nullptr && node->right == nullptr) {
            return level;
        }
        else if (node->left == nullptr) {
            return level + average_node_depth(node->right, level + 1);
        }
        else if (node->right == nullptr) {
            return level + average_node_depth(node->left, level + 1);
        }
        return level + average_node_depth(node->left, level + 1) + average_node_depth(node->right, level + 1);
    }
