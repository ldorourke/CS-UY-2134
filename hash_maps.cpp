...
bool HashTable<HashedObj>::insert( const HashedObj & x ) {
    HashEntry enter(x, ACTIVE);
    int index = hf(x) % array.size();
    while (array[index].info == ACTIVE) {
        if (x == array[index].element) {
            return false;
        }
...
typename HashTable<HashedObj>::iterator HashTable<HashedObj>::find( const HashedObj & x ) {
    ...
    int index = hf(x) % array.size();
    HashTable<HashedObj>::iterator itr = array.begin() + index;
    while (itr->info != EMPTY) {
        if (itr->element == x && itr->info != DELETED) {
            return itr;
        }
        if (++itr == array.end()) {
            itr = array.begin();
...
void HashTable<HashedObj>::rehash() {
    vector<HashEntry> tempArray = array;
    makeEmpty();
    array.resize(2*array.size());
    for (int i = 0; i < tempArray.size(); i++) {
        if (tempArray[i].info == ACTIVE) {
            insert(tempArray[i].element);
        }
    }
}
